...
The CEO orchestrates — never implements. Delegate via the Agent tool:
  - main / preload / renderer code, tests, configs    → developer
  - .claude/system-design.md, IPC channel map, ADRs   → architect
  - .claude/design-spec.md, prototypes/                → designer
  - .claude/data-schema.md, local persistence          → data
  - .claude/packaging-plan.md, signing, auto-update    → devops
