---
name: common-ios-app-sync
description: CEO does a quick sync for an iOS project — reviews recent changes (commits, new repositories, navigation routes, Info.plist drift, fastlane / Xcode version bumps), updates the strategic knowledge base. Use regularly to stay current on the project. Faster than `brief` — surgical update, not a full refresh.
user-invocable: true
allowed-tools: Read, Grep, Glob, Bash, Write, Edit, Agent
argument-hint: "[topic to focus on]"
---

# CEO Quick Sync — iOS App
...
Read `.claude/ceo-brain.md`. If it doesn't exist, tell the user to run `/common-ios-app-brief` first and stop.
...
- Repository / service inventory drift — new files under `Features/*/Repositories/` or `Core/Networking/` etc.
- Navigation route inventory drift — new `Route` enum cases
- Info.plist drift — new `NSUsageDescription` strings, new `Associated Domains`, new background modes
- `Package.swift` drift — new SPM dependencies
- `fastlane/Fastfile`, `Matchfile` — release pipeline changes
- Xcode version pinned in CI — any bump
- Swift toolchain version — Swift 6 strict concurrency rollout progress
...
- Are there new repositories, services, or domains?
- Have priorities shifted? (new features vs bug fixes vs refactors)
- Any new risks or tech debt introduced?
- Did the packaging story change (new capability, new ASC config, new TestFlight ring)?
...
- **Flag surprises.** If something unexpected showed up — a deprecated capability, a new third-party SDK added, a force-unwrap snuck past the hook somehow — call it out.
