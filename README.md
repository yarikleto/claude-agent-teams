...
| Team | What it does |
| --- | --- |
| [`common-web-app`](./teams/common-web-app) | A **spec-driven** software-engineering org tuned for typical web applications (frontend, backend, full-stack SaaS) — orchestrator + 9 specialized agents (architect, backend, DBA, designer, devops, frontend, manual-qa, researcher, ux-engineer), 9 skills implementing the **Specify → Design → Plan/Tasks → Implement → Verify** loop, and 5 hooks for session start, CEO-delegation check, auto-format, save-progress, and post-commit reminders. Separate backend (test-driven) and frontend engineers own their lanes and self-review — no separate reviewer or tester. Adapted from [yarikleto/claude-swe-plugin](https://github.com/yarikleto/claude-swe-plugin). |
| [`common-electron-app`](./teams/common-electron-app) | The desktop sibling of `common-web-app` — same **Specify → Design → Plan/Tasks → Implement → Verify** loop, retuned for cross-platform **Electron** apps. Orchestrator + 10 specialized agents (architect, data, designer, developer, devops, manual-qa, researcher, reviewer, tester, ux-engineer), 10 skills (including `devops-package` for packaging, signing, notarization & auto-update), and 5 hooks for session start, main/renderer separation check, auto-format, save-progress, and post-commit reminders. **Mutually exclusive with `common-web-app` and `common-ios-app` at the same scope** — they share agent names, so install one or the other per scope. |
| [`common-ios-app`](./teams/common-ios-app) | The native-mobile sibling — same **Specify → Design → Plan/Tasks → Implement → Verify** loop, retuned for **SwiftUI-first iOS apps** (UIKit-aware, Swift 5.9+/6 concurrency). Orchestrator + 10 specialized agents (architect, data, designer, developer, devops, manual-qa, researcher, reviewer, tester, ux-engineer), 11 skills (including `devops-package` for fastlane/match/gym/pilot/deliver + TestFlight + ASC, and `ui-verify` — a simulator-driven UI verification runbook that boots the pinned sim, builds, installs, screenshots, and feeds them back to the ux-engineer), and 5 hooks for session start, MVVM separation check, swiftformat auto-format, save-progress, and post-commit TestFlight-version reminders. No `.mcp.json` — UI verification runs entirely through `xcodebuild` + `xcrun simctl`. **Mutually exclusive with `common-web-app` and `common-electron-app` at the same scope** — they share agent names, so install one or the other per scope. |
