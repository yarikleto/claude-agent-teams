...
| [`common-electron-app`](./teams/common-electron-app) | The desktop sibling of `common-web-app` — same **Specify → Design → Plan/Tasks → Implement → Verify** loop, retuned for cross-platform **Electron** apps. Orchestrator + 10 specialized agents (architect, data, designer, developer, devops, manual-qa, researcher, reviewer, tester, ux-engineer), 10 skills (including `devops-package` for packaging, signing, notarization & auto-update), and 5 hooks for session start, main/renderer separation check, auto-format, save-progress, and post-commit reminders. **Mutually exclusive with `common-web-app` at the same scope** — they share agent names, so install one or the other per scope. |
